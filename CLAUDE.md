...
- **Auth (simulated, MVP only):** Session = HMAC-SHA256 signed cookie (`sk_session`) carrying only `profileId`. Role is **always re-derived from the `users` table per request** in `src/lib/auth.ts` — never read from the cookie payload. This means a forged or tampered cookie cannot elevate privileges. Real Microsoft Entra SSO is out of scope for the MVP and will be implemented post-launch; do not add any real SSO integration without explicit approval.
